...
class MixtureDiscretizedLogistic(tfd.Distribution):
    ...
    def _sample_n(self, n, seed=None):
        ...
        n_logits_reshaped = tf.reshape(n_logits, [self.logits.shape[0] * n, *self.logits.shape[1:]])

        # ---- get samples
        samples = sample_from_discretized_mix_logistic(n_logits_reshaped, self.n_mix)

        # ---- split the sample and batch dimensions
        samples_reshaped = tf.reshape(samples, [n, *self.logits.shape[:-1], 3])

        return samples_reshaped


# ---- the following is adapted from https://github.com/openai/pixel-cnn/blob/master/pixel_cnn_pp/nn.py
